...
function battlePhaseAnimation() {
    ...
    if (battlePhaseStarted) {
        ...
        for (var i = 0; i < computerHand.length; i++) {
            for (var j = 0; j < computerHand.length; j++) {
                if (age[i] == computerHand[j].age) {
                    ageDict[age[i]] = computerHand[j].name
                }
                if (power[i] == computerHand[j].power) {
                    powerDict[power[i]] = computerHand[j].name
                }
                if (magicLevel[i] == computerHand[j].magicLevel) {
                    magicLevelDict[magicLevel[i]] = computerHand[j].name
                }
                if (frightFactor[i] == computerHand[j].frightFactor) {
                    frightFactorDict[frightFactor[i]] = computerHand[j].name
                }
            }
        }
        ...
        if (!highest.includes(ageDict[age[0]])) {
            highest[0] = ageDict[age[0]]
        }
        else {
            if (!highest.includes(ageDict[age[1]])) {
                highest[0] = ageDict[age[1]]
            }
            else {
                if (!highest.includes(ageDict[age[2]])) {
                    highest[0] = ageDict[age[2]]
                }
                else {
                    highest[0] = ageDict[age[3]]
                }
            }
        }

        if (!highest.includes(powerDict[power[0]])) {
            highest[1] = powerDict[power[0]]
        }
        else {
            if (!highest.includes(powerDict[age[1]])) {
                highest[1] = powerDict[power[1]]
            }
            else {
                if (!highest.includes(powerDict[age[2]])) {
                    highest[1] = powerDict[power[2]]
                }
                else {
                    highest[1] = powerDict[power[3]]
                }
            }
        }

        if (!highest.includes(magicLevelDict[magicLevel[0]])) {
            highest[2] = magicLevelDict[magicLevel[0]]
        }
        else {
            if (!highest.includes(magicLevelDict[magicLevel[1]])) {
                highest[2] = magicLevelDict[magicLevel[1]]
            }
            else {
                if (!highest.includes(magicLevelDict[magicLevel[2]])) {
                    highest[2] = magicLevelDict[magicLevel[2]]
                }
                else {
                    highest[2] = magicLevelDict[magicLevel[3]]
                }
            }
        }
    
        if (!highest.includes(frightFactorDict[frightFactor[0]])) {
            highest[3] = frightFactorDict[frightFactor[0]]
        }
        else {
            if (!highest.includes(frightFactorDict[frightFactor[1]])) {
                highest[3] = frightFactorDict[frightFactor[1]]
            }
            else {
                if (!highest.includes(frightFactorDict[frightFactor[2]])) {
                    highest[3] = frightFactorDict[frightFactor[2]]
                }
                else {
                    highest[3] = frightFactorDict[frightFactor[3]]
                }
            }
        }
